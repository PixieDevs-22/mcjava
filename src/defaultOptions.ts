/**
 * Detect low-end / mobile devices at startup so we can pick sane defaults.
 * Uses coarse pointer (touch screen), userAgent, hardware concurrency, and
 * device memory (non-standard but available on Android Chrome).
 */
export const detectMobileOrLowEnd = (): boolean => {
  ...
  return coarsePointer || mobileUA || (lowCores && lowMemory)
}

const _isMobileOrLowEnd = detectMobileOrLowEnd()

export const defaultOptions = {
  renderDistance: _isMobileOrLowEnd ? 5 : 3,
  keepChunksDistance: _isMobileOrLowEnd ? 0 : 1,
  multiplayerRenderDistance: _isMobileOrLowEnd ? 5 : 3,
  ...
  gpuPreference: (_isMobileOrLowEnd ? 'low-power' : 'default') as 'default' | 'high-performance' | 'low-power',
  backgroundRendering: '5fps' as 'full' | '20fps' | '5fps',
  /** @unstable */
  disableAssets: false,
  /** @unstable */
  ...
  loadPlayerSkins: !_isMobileOrLowEnd,
  renderEars: true,
  lowMemoryMode: _isMobileOrLowEnd,
