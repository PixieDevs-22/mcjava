...
// sharp is optional — only needed for SINGLE_FILE_BUILD to resize the favicon
let sharp: any
                    ...
                    build.onAfterBuild(async () => {
                        ...
                        if (SINGLE_FILE_BUILD) {
                            ...
                            const resizedImage = sharp
                                ? (await sharp('./assets/favicon.png').resize(64).toBuffer()).toString('base64')
                                : fs.readFileSync('./assets/favicon.png', 'base64')
