...
import { detectMobileOrLowEnd } from '../../../src/defaultOptions'
...
export class DocumentRenderer {
  ...
  constructor (initOptions: GraphicsInitOptions, public externalCanvas?: OffscreenCanvas) {
    ...
    try {
      // On mobile/low-end: disable logarithmicDepthBuffer (GPU expensive) and antialias
      // These settings save ~20-30% GPU time on mobile with no visible difference at rd=5
      this.renderer = new THREE.WebGLRenderer({
        canvas: this.canvas,
        preserveDrawingBuffer: true,
        logarithmicDepthBuffer: !_isMobileOrLowEnd,
        antialias: !_isMobileOrLowEnd,
        powerPreference: this.config.powerPreference ?? (_isMobileOrLowEnd ? 'low-power' : 'default')
      })
    } catch (err) {
  ...
  updatePixelRatio () {
    ...
    // Cap pixel ratio on mobile to 1.5 — retina at full 3x costs 9x the GPU fill rate
    if (_isMobileOrLowEnd) {
