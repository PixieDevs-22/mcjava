...
export const defaultWorldRendererConfig = {
  ...
  mesherWorkers: _isMobileOrLowEnd ? 2 : 4,
  // Reduce batch wait on mobile so chunks stream in faster at rd=5
  addChunksBatchWaitTime: _isMobileOrLowEnd ? 80 : 200,
  _experimentalSmoothChunkLoading: true,
  _renderByChunks: false,

  // Rendering engine settings
  dayCycle: true,
  smoothLighting: !_isMobileOrLowEnd,
  cardinalLight: 'default',
  enableLighting: true,
  starfield: !_isMobileOrLowEnd,
  defaultSkybox: true,
  renderEntities: !_isMobileOrLowEnd,
...
export abstract class WorldRendererCommon<WorkerSend = any, WorkerReceive = any> {
  ...
  // On mobile we yield to the render thread sooner to keep frame time low
  ONMESSAGE_TIME_LIMIT = _isMobileOrLowEnd ? 12 : 30 // ms
